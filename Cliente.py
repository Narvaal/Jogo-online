...
def Receber():
    ...
    print ("[SERVIDOR]Data recebida: " + str(data))  
    try:
    ...
    return data,posx,posy,velox,veloy

def Enviar (env):
    MENSAGEM  = env
    print("[CLIENTE]Data envaida" + env) 
...
while Rodando:
    ...
    Enviar(str(P1_pos_x) + "," + str(Bola.x) + "," + str(Bola.y))
    data,posx,posy,velox,veloy = Receber()
    P2_pos_x = data

    velocidade_bola_x = velox
    velocidade_bola_y = veloy
    ...
    #Colisão  teto
    if Player1.colliderect(Alto_coli) and flag_cima:
        flag_cima = False
        velocidade_x = 0
    elif Player1.colliderect(Baixo_coli) and flag_baixo:
        flag_baixo = False
        velocidade_x = 0
